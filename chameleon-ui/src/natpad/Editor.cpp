...
void Editor::on_property_value_vadjustment() {
	...
	if (vertical_adjustment) {
		co = vertical_adjustment->signal_value_changed().connect(sigc::mem_fun(*this, &Editor::l_set_vadjustment));
		vertical_adjustment->set_upper(8000);
		vertical_adjustment->set_page_size(80);
	}
	if (view) {
		view->setVerticalAdjustment(vertical_adjustment);
	}
}
...
void Editor::l_set_vadjustment() {
	...
	long long old = view->set_view_y(aval);
